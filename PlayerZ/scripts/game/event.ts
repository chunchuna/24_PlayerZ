...
class GUARD {
    static 1() {
        ...
        var GameVariblesInstance: InstanceType.GameVariables = ENGINE_MUST.CORE.objects.GameVariables.getFirstInstance()!;
        if (GameVariblesInstance == null) return;
        var map = GameVariblesInstance.getDataMap();

        //@ts-ignoreleta
        if (map.get("GuardDie") == 1) return;
        //@ts-ignoreleta
        ...
        if (map.get("GetOutRoom") == 0) {
            if (map.get("GetGold") == 1) { StartDialogue("Guard_AboutGold", 26) }
            //@ts-ignoreleta
            if (map.get("GetGold") == 0) { StartDialogue("Guard", 26); }
        }
    }
    ...
    static 2() {
        ...
        //@ts-ignoreleta
        var Guard: InstanceType.guard = ENGINE_MUST.CORE.getInstanceByUid(26)!;
        if (Guard == null) return;

        //@ts-ignoreleta
        var NudeGuard: InstanceType.guard = ENGINE_MUST.CORE.getInstanceByUid(130)!;
        if (NudeGuard == null) return;
        ...
        //@ts-ignoreleta
        var FakeGuardSprite: InstanceType.guardbutfake = ENGINE_MUST.CORE.getInstanceByUid(88)!;
        //@ts-ignoreleta
        var PrisonerMainSprite: InstanceType.prisoner3dobject = ENGINE_MUST.CORE.getInstanceByUid(17)!;
...
class GOLD {
    ...
    static 2() {
        //@ts-ignoreletada
        var Suitcase: InstanceType.Suitcase = ENGINE_MUST.CORE.getInstanceByUid(124);
        //@ts-ignoreleta
        var PrisonerMain: InstanceType.prisonerMain = ENGINE_MUST.CORE.objects.prisonerMain.getFirstInstance();

        if (PrisonerMain == null || Suitcase == null) return;
...
class GATE {
    static 1() {
        ...
        // 狱警开门
        //@ts-ignoreleta
        var Gate1: InstanceType.door = ENGINE_MUST.CORE.getInstanceByUid(24)!;
        //@ts-ignoreleta
        var Gate2: InstanceType.door = ENGINE_MUST.CORE.getInstanceByUid(3)!;
...
class AMEIRICAN_GUARD {
    ...
    static 1() {
        ...
        if (map.get("GOpenGate2") == 1) { return; }
